...
//          Map<String, List<String>> map = connection.getHeaderFields();// 获取所有响应头字段
    ...
    private static String sk = "dAJuAtMbe3o5Ekj8jhMivqUZZButtVxvt7ZNXqKNPxA=";

    public static double getUsdtPrice(){
        String method = "/coin/exchange";
        String param = "shortName=CNY&accessKey=" + URLEncoder.encode(sk);
        String returnJson = sendGet(method, param);
        return JSONObject.parseObject(returnJson).getJSONObject("data").getDoubleValue("USDT");
    }

    public static double getBmjPrice(){
        String method = "/market/ticker";
        String param = "symbol=bmjusdt&accessKey=" + URLEncoder.encode(sk);
        String returnJson = BzuApiUtils.sendGet(method, param);
        return JSONObject.parseObject(returnJson).getJSONObject("data").getDoubleValue("sell")*getUsdtPrice();
    }
